...
describe('createWakeEntity', () => {
  ...
  it('attaches a mesh to the scene', () => {
    const scene = { add: vi.fn(), createPoints: vi.fn().mockReturnValue({ dispose: vi.fn() }), createBufferGeometry: vi.fn().mockReturnValue({}), setAttribute: vi.fn(), setIndex: vi.fn() } as any;
    const entity = createWakeEntity();
